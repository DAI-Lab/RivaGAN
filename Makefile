...
test: ## run tests quickly with the default Python
	python -m pytest --basetemp=${ENVTMPDIR} --cov=rivagan

.PHONY: lint
lint: ## check style with flake8 and isort
	flake8 rivagan tests
	isort -c --recursive rivagan tests
...
fix-lint: ## fix lint issues using autoflake, autopep8, and isort
	find rivagan -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive rivagan
	isort --apply --atomic --recursive rivagan
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source rivagan -m pytest
	coverage report -m
	coverage html
...
docs: clean-docs ## generate Sphinx HTML documentation, including API docs
	sphinx-apidoc --separate --no-toc -o docs/api/ rivagan
	$(MAKE) -C docs html
