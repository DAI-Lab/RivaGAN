...
class JpegCompression(nn.Module):
    ...
    def forward(self, noised_image):
        N, _, L, H, W = noised_image.size()
        noised_image = noised_image.permute(0, 2, 1, 3, 4).view(-1, 3, H, W)
        ...
        # un-pad
        noised_image = image_ret_padded[:, :, :image_ret_padded.shape[2]-pad_height, :image_ret_padded.shape[3]-pad_width].clone()

        noised_image = noised_image.view(N, L, 3, H, W).permute(0, 2, 1, 3, 4)
        return noised_image
