...
RivaGAN could always use more documentation, whether as part of the
official RivaGAN docs, in docstrings, or even on the web in blog posts,
articles, and such.
...
Ready to contribute? Here's how to set up `RivaGAN` for local development.
...
2. The tests that cover a module called ``rivagan/path/to/a_module.py``
   should be implemented in a separated module called
   ``tests/rivagan/path/to/test_a_module.py``.
...
To run a subset of tests::
    ...
    $ python -m pytest tests.test_rivagan
...
2. Update the version in ``setup.cfg``, ``rivagan/__init__.py`` and
   ``HISTORY.md`` files.
3. Create a new git tag pointing at the corresponding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``rivagan/__init__.py``
